...
# Enabling orientation sensor gyroscope and stability classifier
...
def bypassedCriticalTipping():
    global buzz
    buzz.value = True
    # LED shows A
    leftBrakingCont(9415)
    rightBrakingCont(9415)
    time.sleep(10)
    buzz.value = False
...
while True:
    ...
    # Replace current angle and tipping angle with value.
    if current angle >= tipping angle:
        bypassedCriticalTipping()

    # Soft-start will turn off when on incline. 45 is temporary, until good values is found.
    if gyro_y < 30:
        softStartCheck(False)
    else:
        softStartCheck(True)
    ...
    if gearFlag == 3:
        ...
        if y > 132:
            if x < 120:
                ...
                xAxisToPWM = int(map_range(x, 136, 255, 6150, 7000))
                ...
                # LB = 6050
                xAxisToPWM = int(map_range(x, 120, 0, 6150, 7000))
                targFor1PWM = xAxisToPWM
                targFor2PWM = xAxisToPWM
                leftMotorCont(currFor1PWM)
                rightMotorCont(currFor2PWM)
    ...
    # When gearFlag = 0, 1 or 2, the user can drive normally and steer.
    elif gearFlag == 0 or gearFlag == 1 or gearFlag == 2:
        ...
        if y > 132:
            ...
            if x < 120:
                ...
                xAxisToPWM = int(map_range(x, 120, 0, 200, 400))
                ...
                xAxisToPWM = int(map_range(x, 136, 255, 200, 400))
    ...
    # When gearFlag = 0, 1 or 2, the user can drive normally and steer.
        ...
        if y > 132:
            ...
            if x < 120:
                # Slow down left motor to steer left based on the position of joystick on x-axis
                xAxisToPWM = int(map_range(x, 120, 0, 800, 1200))
                targFor1PWM = yAxisToPWM
                targFor2PWM = yAxisToPWM - xAxisToPWM
                if targFor1PWM < 5800:
                    targFor1PWM = 5800
                ...
                # Slow down right motor to steer right.
                xAxisToPWM = int(map_range(x, 136, 255, 800, 1200))
                targFor1PWM = yAxisToPWM - xAxisToPWM
                targFor2PWM = yAxisToPWM
                if targFor1PWM > 7000:
                    targFor1PWM = 7000
        ...
        if y > 132:
            ...
            if x < 120:
                # Slow down left motor to steer left based on the position of joystick on x-axis
                xAxisToPWM = int(map_range(x, 120, 0, 1800, 2400))
                targFor1PWM = yAxisToPWM
                targFor2PWM = yAxisToPWM - xAxisToPWM
                if targFor1PWM < 5800:
                    targFor1PWM = 5800
                ...
                # Slow down right motor to steer right.
                xAxisToPWM = int(map_range(x, 136, 255, 1800, 2400))
                targFor1PWM = yAxisToPWM - xAxisToPWM
                targFor2PWM = yAxisToPWM
                if targFor1PWM > 8200:
                    targFor1PWM = 8200
        ...
        if y > 132:
            if x < 120:
                ...
                pwm_6.duty_cycle = 9415
                # Zero-point turns to the left.
                xAxisToPWM = int(map_range(x, 136, 255, 6150, 8200))
                targFor1PWM = xAxisToPWM
                targFor2PWM = xAxisToPWM
                leftMotorCont(currFor1PWM)
                rightMotorCont(currFor2PWM)
                ...
                # Zero-point turns to the right.
                # LB = 6050
                xAxisToPWM = int(map_range(x, 120, 0, 6150, 8200))
                targFor1PWM = xAxisToPWM
                targFor2PWM = xAxisToPWM
                leftMotorCont(currFor1PWM)
                rightMotorCont(currFor2PWM)
